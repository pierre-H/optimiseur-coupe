...
// le dernier élément du tableau doit être un 0
List * createSortedList (List * l, double tabLongueurs [])
{
	int i = 0;
	while (tabLongueurs [i] != 0)
	{
		l->trie (tabLongueurs [i]);
		i++;
	}
	return l;
}

// Idem avec un QVector de saisie en second argument
List * createSortedList (List * l, QVector<Saisie *> vecteur)
{
    int i,j;
    for(i=0; i<vecteur.size();i++){
        for(j=0; j < vecteur[i]->quantite() ; j++)
            l->trie(vecteur[i]->longueur());
    }
    return l;
}
...
#else
QString List::toStr(bool premier) const
{
    QString text("");
    if (not premier)
    {
        text += ConvertUnit::toStrSimplifie(m_element);
        if (m_prochain != NULL)
        {
            text += ", " + m_prochain->toStr(false);
        }
        else
            text += "<br>";       // Fin de liste
    }
    else    // premier élément de la liste ou liste vide
    {
        if (m_prochain)
...
int main ()
{
	...
	double tab []= {12.,18.2, 40.5, 95.,54., 546.,45., 0};
	List * li = new List(); createSortedList (li,tab);
	li->affiche ();
